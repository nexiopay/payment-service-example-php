...
<head>
    <meta charset="UTF-8">
    <title>Run eCheck Transaction Example</title>
    <style>
        .main {
            display: block;
            height: 900px;
    ...
    window.addEventListener('message', function messageListener(event) {
        if (event.origin === iframeDomain) {
            ...
            if (event.data.event === 'processed') {
                ...
                window.document.getElementById('forms-container').innerHTML = '<p>Successfully Processed eCheck Transaction.</p><code><br/>' + jsonStr + '</code>';
            }
        }
    });
