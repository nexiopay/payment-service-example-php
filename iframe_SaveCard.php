...
<head>
    <meta charset="UTF-8">
    <title>Save Card Transaction Example</title>
    <style>
        .main {
            display: block;
            height: 900px;
...
<script>
    ...
    const iframeUrl = '<?php echo $apiurl."pay/v3/saveCard"; ?>';
    ...
    window.addEventListener('message', function messageListener(event) {
        if (event.origin === iframeDomain) {
            ...
            if (event.data.event === 'cardSaved') {
                ...
                window.document.getElementById('forms-container').innerHTML = '<p>Successfully Processed Save Card Transaction.</p><code><br>' + jsonStr + '</code>';
            }
        }
    });

    function setup() {
		fetch('GetTokenSaveCard.php').then(function (response) {
